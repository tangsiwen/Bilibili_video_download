...
def get_play_list(aid, cid, quality):
    ...
    headers = {
        ...
        'Cookie': 'SESSDATA=03203f7a%2C1725850095%2Cc24b3%2A32CjBs_o5RVO9FmLItERI-1kDWr7DED3u8N-AhekwU3D_jxXNazHHLpGcm7GAkfJAlLIcSVjhwY0dUcjNJWExQTnpUOVhRWlc2SWM5LWFZY3kweUJuR2I5UnlDZ1BJR2dSTmtVN05VdzczemNrNHJBUlpFOWF1S1dDSTdCRHIzYXZBSzZKZWFqOHN3IIEC', # 登录B站后复制一下cookie中的SESSDATA字段,有效期1个月
...
#  下载视频
def down_video(video_list, rootTitle ,title, start_url, page):
    num = 1
    print('[正在下载P{}段视频,请稍等...]:'.format(page) + title)
    currentVideoPath = os.path.join(sys.path[0], rootTitle, title)  # 当前目录作为下载目录
    for i in video_list:
...
# 合并视频
def combine_video(video_list, rootTitle ,title):
    currentVideoPath = os.path.join(sys.path[0], rootTitle, title)  # 当前目录作为下载目录
    if len(video_list) >= 2:
        # 视频大于一段才要合并
...
if __name__ == '__main__':
    ...
    print('[下载视频的aid]:' + str(aid))
    print('[当前BV的标题]:' + rootTitle)
    cid_list = []
    if '?p=' in start:
        # 单独下载分P视频中的一集
    ...
    for item in cid_list:
        ...
        down_video(video_list, rootTitle, title, start_url, page)
        combine_video(video_list, rootTitle ,title)
